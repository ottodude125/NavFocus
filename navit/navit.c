...
navit_init(struct navit *this_)
{
	...
	if (this_->navigation) {
		...
		//SDP Added
		struct callback * mine = callback_new(calculateHeading,0,NULL);
		navigation_register_callback(this_->navigation,attr_navfocus_update,mine);
		//SDP End Added
		if (this_->route)
